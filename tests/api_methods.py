...
reports=[]
for dim in [2,3]:
    for method in ['SIMP','BESO','ESO','level-set']:
        settings=dict(dim=dim,nx=12,ny=8,nz=2,vf=.6,er=.08,radius=2,penalty=3,maxIter=3,method=method)
        ...
        for f in frames:
            ...
            assert all(.001-1e-10<=x<=1+1e-10 for x in f['density'])
...
for invalid in [dict(method='wrong'),dict(moveLimit=0),dict(timeStep=1),dict(regularization=-1)]:
