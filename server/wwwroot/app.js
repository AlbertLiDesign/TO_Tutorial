import './tutorial.js?v=11';
import {showPane} from './layout.js?v=11';
import {t, meshText, applyLanguage, toggleLanguage} from './i18n.js?v=11';
...
function settings(){let p={dim,method:$('#method').value,besoKill:$('#besoKill').value};for(const f of fields)p[f]=Number($('#'+f).value);p.vf/=100;p.er/=100;return p;}
function modelSettings(){return activeSettings??settings();}
function updateForm(){let p=settings();document.querySelectorAll('[data-methods]').forEach(label=>{label.hidden=!label.dataset.methods.split(' ').includes(p.method);label.querySelector('input,select').disabled=label.hidden;});$('#penalty').closest('label').hidden=p.method==='level-set'||(p.method==='BESO'&&p.besoKill==='hard');$('#penalty').disabled=$('#penalty').closest('label').hidden;$('#vf-value').textContent=p.vf*100+'%';$('#depth-label').hidden=dim===2;$('#mesh-info').textContent=meshText(p.nx*p.ny*(dim===3?p.nz:1),dim);document.querySelectorAll('[data-mesh]').forEach(b=>b.classList.toggle('active',b.dataset.mesh===`${p.nx},${p.ny}`));if(!activeSettings)render();}
...
$('#besoKill').onchange=$('#method').onchange=()=>{if(running)return;if($('#method').value==='level-set'&&$('#maxIter').value==='100')$('#maxIter').value='300';activeSettings=null;frames=[];current=-1;runId=null;setState('ready');updateForm();};
