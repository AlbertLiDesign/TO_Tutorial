...
function drawScene(p,f){if(!$('#scene').clientWidth||!$('#scene').clientHeight)return;if(!Number.isInteger(p.nx)||!Number.isInteger(p.ny)||!Number.isInteger(p.nz)||p.nx<4||p.ny<4||p.nz<1||p.nx>200||p.ny>150||p.nz>32||p.nx*p.ny*(p.dim===3?p.nz:1)>32000)return;const[c,w,h]=canvas($('#scene'));c.fillStyle=ink('canvas-paper');c.fillRect(0,0,w,h);c.fillStyle=ink('canvas-dots');for(let x=12;x<w;x+=20)for(let y=12;y<h;y+=20){c.beginPath();c.arc(x,y,.6,0,Math.PI*2);c.fill();}let values=f?.sensitivity,max=values?.reduce((a,b)=>Math.max(a,b),0)??1;const color=i=>sensitivity&&values?heat(Math.pow(Math.max(0,values[i]/(max||1)),.35)):ink('material');
if(p.dim===2){const scale=Math.min((w-115)/p.nx,(h-Math.min(70,h*.3))/p.ny),left=(w-p.nx*scale)/2,top=(h-p.ny*scale)/2;
 ...
 arrow(c,left+p.nx*scale+7,top+(1-p.loadY)*p.ny*scale,p.force<0?1:-1,Math.min(38,h*.25));if(h>150){c.font='12px sans-serif';c.fillStyle=ink('canvas-label');c.textAlign='center';c.fillText(`${p.nx}`,left+p.nx*scale/2,top+p.ny*scale+24);c.save();c.translate(left-28,top+p.ny*scale/2);c.rotate(-Math.PI/2);c.fillText(`${p.ny}`,0,0);c.restore();}return;}
