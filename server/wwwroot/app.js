import './tutorial.js?v=10';
import {showPane} from './layout.js?v=10';
import {t, meshText, applyLanguage, toggleLanguage} from './i18n.js?v=10';
const $=s=>document.querySelector(s), fields=['nx','ny','nz','vf','er','radius','penalty','maxIter','young','nu','force','loadY','moveLimit','timeStep','regularization'];
...
function settings(){let p={dim,method:$('#method').value};for(const f of fields)p[f]=Number($('#'+f).value);p.vf/=100;p.er/=100;return p;}
function modelSettings(){return activeSettings??settings();}
function updateForm(){let p=settings();document.querySelectorAll('[data-methods]').forEach(label=>{label.hidden=!label.dataset.methods.split(' ').includes(p.method);label.querySelector('input').disabled=label.hidden;});$('#vf-value').textContent=p.vf*100+'%';$('#depth-label').hidden=dim===2;$('#mesh-info').textContent=meshText(p.nx*p.ny*(dim===3?p.nz:1),dim);document.querySelectorAll('[data-mesh]').forEach(b=>b.classList.toggle('active',b.dataset.mesh===`${p.nx},${p.ny}`));if(!activeSettings)render();}
...
$('#method').onchange=()=>{if(running)return;activeSettings=null;frames=[];current=-1;runId=null;setState('ready');updateForm();};
...
if(p.dim===2){const scale=Math.min((w-115)/p.nx,(h-Math.min(70,h*.3))/p.ny),left=(w-p.nx*scale)/2,top=(h-p.ny*scale)/2;
 ...
 for(let x=0;x<p.nx;x++)for(let y=0;y<p.ny;y++){const i=x*p.ny+y;if(!f||sensitivity||p.method==='SIMP'||f.density[i]>.5){c.fillStyle=color(i);c.fillRect(left+x*scale,top+(p.ny-y-1)*scale,scale+.2,scale+.2);}}
